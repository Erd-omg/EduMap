...
└──────────┬──────────────────────────────────────────────────────────┘
           ...
           ▼                          ▼
┌──────────────────────┐  ┌──────────────────────────────────────────┐
│  profile-service     │  │          backend-core (FastAPI)          │
│  (:8001)             │  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ │
│  画像分析 SSE 端点    │  │  │Orchestr. │ │ Planner  │ │ Guardian │ │
│  用户画像持久化       │  │  └──────────┘ └──────────┘ └──────────┘ │
└──────────┬───────────┘  │  ┌──────────┐ ┌──────────┐ ┌────────────┐│
           │              │  │ Designer │ │  Coder   │ │  Mentor    ││
           │              │  └──────────┘ └──────────┘ │ (RAG QA)   ││
           │              │  ┌──────────┐ ┌────────────┐└────────────┘│
           │              │  │Assessment│ │Content     │              │
           │              │  │  Agent   │ │ Auditor    │              │
...
### 3.8 智能辅导（Phase 5 已实现 ✅）

Mentor Agent 已作为 Phase 5 的核心交付完成开发：
- 集成到对话页面（`/chat`），与画像分析共享同一界面，用户通过模式切换 Tab 选择
- 即时多模态答疑，RAG 约束生成
- 混合检索（Neo4j 精确事实 + ChromaDB 语义片段 + 上传文档解析内容）
- 右侧面板展示来源引用（SourcePanel），支持展开查看摘要
