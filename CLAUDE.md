...
**区分两类"多值断言"**（2026-09-23 核查 15 处后的结论）：

- **三值及以上如 `(200, 422, 500)`——必须改。** 它接受任何结果，包括服务端错误，等于没有断言。全仓原有 3 处，已全部收紧为具体值 + 契约断言。
- **两值如 `(200, 404)`——多数是合理的**，用于断言"两个合法分支之一"（例如"有推荐 / 无推荐"）。**不要机械地把它们都改成单值**：`next-reco` 在 mock 下确实返回 404（空路径），改成 `== 200` 就是错的。

**判断方法：先实测再收紧。** 用 `TestClient` 直接打一次该端点看真实状态码，再决定断言什么。**不要凭名字或直觉猜**——`test_get_path_returns_200` 名字承诺 200 却接受 404，实测确为 200，收紧后才名副其实。往断言里填一个没验证过的具体值，只是把一种猜换成了另一种猜。
...
写在这里避免被重新发现或误以为已修。**下面都是仍未解决的问题**（已修复的见「历史修复」一节）。
...
- **前后端共享类型未校验**：`packages/shared-types` 定义了响应契约，但没有任何测试验证后端响应符合它。目前靠各用例手写断言，字段改名未必被发现。

## 历史修复（不要回退这些不变量）

以下问题已修复，**但对应约束不能改坏**——每条都有变异体或测试守着。

### 熔断器三态（CLOSED / OPEN / HALF_OPEN）

恢复窗口到期后进入 HALF_OPEN，**只放行一个探针**；探针成功 → CLOSED，失败 → **立即重新熔断**并重置完整窗口。实测持续故障 10 秒：改前 9 次无效调用 → 改后 6 次（每周期 1 探针）。

**三条不可破坏的约束：**

1. **`_try_acquire_slot` 内不得出现 `await`。** 它靠"单事件循环 + 检查与置位之间无 await"保证原子性。加了 await 就会让多个调用者同时赢得探针。`TestCircuitBreakerConcurrency` 守这条。
2. **`_is_circuit_open()` / `circuit_breaker_state()` 必须是纯读，不能占用探针槽。** 健康检查会调它们；一旦占用，一次 `/health/ready` 就会偷走本该给真实请求的探针，熔断器永远无法闭合。
3. **两处释放路径必须保留**：`_call_with_retry_and_cb` 的 `try/except BaseException` 与 `_call_llm_stream` 的 `finally`。它们保证异常、取消、消费者提前中断（GeneratorExit）时释放探针槽——否则熔断器**永久卡在 HALF_OPEN**，上游恢复后也无法探测。

### 测试进程内加载真实嵌入模型的挂起

三次挂起，同一根因：测试路径加载真实的 `SentenceTransformer`，进程内加载慢且不稳定（有时秒过、有时挂死 >20 分钟），全量跑是否挂取决于执行顺序，因此长期未被发现。
...
- 全量测试现在约 **60–70 秒**。**已配 60 秒单测超时**（`pytest.ini` 的 `timeout = 60`），所以挂起会**在 60 秒后自动失败并打印栈**，精确定位到文件和行号，而不是无限静默。若某个正当的慢测试需要更长时间，用 `@pytest.mark.timeout(N)` 单独放宽，不要调高全局值。
- **不要用百分比估算卡在哪个用例**——我按 dot-stream 百分比猜了三次，三次都错。正确做法：`pytest -v`，然后找**最后一条没有 PASSED/FAILED verdict 的 `::` 行**；但注意 pytest 会缓冲输出，这一行可能是"刚启动"而非"卡住"，需结合耗时判断。**现在有了超时，通常直接等它报错即可，不必手工排查。**
- **最有效的定位手段是按目录分别跑**（`for d in tests/*/; do pytest $d; done`），每个目录单独跑通常都能跑完，从而快速排除大部分范围。
- **`--timeout` 已在 `pytest.ini` 中全局配置**，命令行不需要再传；若要临时关闭用 `--timeout=0`。
