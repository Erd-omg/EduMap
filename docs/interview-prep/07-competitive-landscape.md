...
**仍未做的**：参数尚未用真实数据拟合。**管道已验证可用**（两次 `POST /progress` → 落库 2 条，`event_type` 正确区分 learn/review），但 `forgetting_review_log` 当前 **0 条** —— 缺的是真实使用产生的数据，不是代码。

重拟合的**触发判据已明确**：`--source real` 要求 **≥500 个观测点**，低于此数只打警告并声明"不要据此改生产参数"（把 21 个参数拟合到几十个观测点上是噪声；对比 srs-benchmark 用了约 3.5 亿次复习）。数据不足时以 **exit 2** 退出并说明缺什么。

当前参数是"有依据的默认值 + 合成数据验证"，**不是拟合结果**。
